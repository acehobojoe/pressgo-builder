...
17. LAYOUT VARIETY — never give two ADJACENT sections the same skeleton. If the previous section was a centered 3-icon row, this one must be different: a 2-column image+text split, an asymmetric feature row (image on one side), a stat band (3-4 big numbers + labels), an overlapping card, or a quote block. Centered-text-on-a-band is allowed ONLY for the hero, a single focused CTA, or a short pull-quote — NOT for benefits / features / services / about (those want a split or a card grid with real imagery). When in doubt, alternate: split → grid → split. A page of stacked centered text reads as a template.
